 ...
 * <p>
 */
public abstract class GJAbstractLine2D extends GJAbstractSmoothCurve2D
implements GJSmoothOrientedCurve2D, GJLinearElement2D {
    ...
    public static GJPoint2D getIntersection(GJAbstractLine2D line1,
		...
		return new GJPoint2D(line1.x0 + t * line1.dx, line1.y0 + t * line1.dy);
	}

	/**
	 * Tests if the two linear objects are located on the same straight line.
	 */
	public static boolean isColinear(GJAbstractLine2D line1, GJAbstractLine2D line2) {
		...
		return (Math.abs((line2.y0 - line1.y0) * line2.dx
				...
				/ Math.hypot(line2.dx, line2.dy) < GJShape2D.ACCURACY);
	}

    /**
     * Tests if the two linear objects are parallel.
     */
	public static boolean isParallel(GJAbstractLine2D line1, GJAbstractLine2D line2) {
		return (Math.abs(line1.dx * line2.dy - line1.dy * line2.dx) < GJShape2D.ACCURACY);
	}
    ...
    protected GJAbstractLine2D(double x0, double y0, double dx, double dy) {
        this.x0 = x0;
        this.y0 = y0;
        this.dx = dx;
        this.dy = dy;
    ...
    protected GJAbstractLine2D(GJPoint2D point, GJVector2D vector) {
        this.x0 = point.x();
        this.y0 = point.y();
        this.dx = vector.x();
        this.dy = vector.y();
     ...
     */
	public boolean isColinear(GJLinearShape2D linear) {
		// test if the two lines are parallel
		...
		if (Math.abs(dx) > Math.abs(dy)) {
			if (Math.abs((line.x0 - x0) * dy / dx + y0 - line.y0) > GJShape2D.ACCURACY)
				return false;
			else
				return true;
		} else {
			if (Math.abs((line.y0 - y0) * dx / dy + x0 - line.x0) > GJShape2D.ACCURACY)
				return false;
			else
				return true;
		}
    ...
    public boolean isParallel(GJLinearShape2D line) {
        return GJVector2D.isColinear(this.direction(), line.direction());
    }

	/**
	 * Returns true if the point (x, y) lies on the line covering the object,
	 * with precision given by GJShape2D.ACCURACY.
	 */
	protected boolean supportContains(double x, double y) {
		double denom = Math.hypot(dx, dy);
		if (denom < GJShape2D.ACCURACY)
			throw new GJDegeneratedLine2DException(this);
		return (Math.abs((x - x0) * dy - (y - y0) * dx) / denom < GJShape2D.ACCURACY);
	}
    ...
    public double positionOnLine(GJPoint2D point) {
     ...
     */
    public GJPoint2D projectedPoint(GJPoint2D p) {
	 ...
	 * @return the projection of the point p on the line
	 */
	public GJPoint2D projectedPoint(double x, double y) {
		if (contains(x, y))
     ...
     * @return the projection of the point p on the line
     */
    public GJPoint2D getSymmetric(GJPoint2D p) {
     ...
     */
    public GJPoint2D getSymmetric(double x, double y) {
		// compute position on the line
		double t = 2 * positionOnLine(x, y);

		// compute position of intersection point
		return new GJPoint2D(2 * x0 + t * dx - x, 2 * y0 + t * dy - y);
	}
    ...
    public GJStraightLine2D parallel(GJPoint2D point) {
        return new GJStraightLine2D(point, this.dx, this.dy);
    }
    ...
    public GJStraightLine2D perpendicular(GJPoint2D point) {
        return new GJStraightLine2D(point, -this.dy, this.dx);
    }
   ...
   // ===================================================================
    ...
    public GJPoint2D intersection(GJLinearShape2D line) {
        GJVector2D vect = line.direction();
        double dx2 = vect.x();
        double dy2 = vect.y();
		...
		// compute position of intersection point
		GJPoint2D point = new GJPoint2D(x0 + t * dx, y0 + t * dy);
    ...
    /* (non-Javadoc)
	 * @see math.geom2d.line.GJAbstractLine2D#supportingLine()
	 */
	...
	public GJCirculinearElement2D transform(GJCircleInversion2D inv) {
        ...
        GJPoint2D po 	= line.projectedPoint(center);
        ...
        GJPoint2D p1 = inf0 ? center : this.firstPoint().transform(inv);
        GJPoint2D p2 = inf1 ? center : this.lastPoint().transform(inv);
    ...
    public double windingAngle(GJPoint2D point) {
        ...
        if (t0==Double.NEGATIVE_INFINITY)
            angle1 = GJAngle2D.horizontalAngle(-dx, -dy);
        else
			angle1 = GJAngle2D.horizontalAngle(point.x(), point.y(),
					x0 + t0 * dx, y0 + t0 * dy);

		if (t1 == Double.POSITIVE_INFINITY)
			angle2 = GJAngle2D.horizontalAngle(dx, dy);
		else
			angle2 = GJAngle2D.horizontalAngle(point.x(), point.y(),
					x0 + t1 * dx, y0 + t1 * dy);
     ...
     * This method is not designed to be used directly, because GJAbstractLine2D
	...
	/* (non-Javadoc)
	 * @see math.geom2d.curve.GJSmoothCurve2D#tangent(double)
	 */
     ...
     */
    @Override
    public Collection<? extends GJAbstractLine2D> smoothPieces() {
    ...
    public Collection<GJPoint2D> intersections(GJLinearShape2D line) {
        ...
        ArrayList<GJPoint2D> points = new ArrayList<GJPoint2D>(1);
		GJPoint2D point = intersection(line);
		if (point != null)
			points.add(point);
	...
	public double position(GJPoint2D point) {
		...
		double eps = Math.hypot(dx, dy) * GJShape2D.ACCURACY;
    ...
    /**
     * Returns a new GJAbstractLine2D, which is the portion of this GJAbstractLine2D
     * delimited by parameters t0 and t1. Casts the result to GJStraightLine2D,
     * GJRay2D or GJLineSegment2D when appropriate.
     */
    public GJAbstractLine2D subCurve(double t0, double t1) {
        ...
        if (Double.isInfinite(t1)) {
            if (Double.isInfinite(t0))
                ...
                return new GJRay2D(this.point(t0), this.direction());
        }

        if (Double.isInfinite(t0))
            return new GJInvertedRay2D(this.point(t1), this.direction());
        else
            return new GJLineSegment2D(this.point(t0), this.point(t1));
     ...
     */
	@Override
	public Collection<? extends GJAbstractLine2D> continuousCurves() {
    	return wrapCurve(this);
    }

    // ===================================================================
    // methods implementing the GJShape2D
    ...
    public double distance(double x, double y) {
        ...
        GJPoint2D proj = projectedPoint(x, y);
   ...
   public boolean contains(GJPoint2D p) {
    ...
    public boolean isEmpty() {
		return Math.hypot(dx, dy) < GJShape2D.ACCURACY;
    }

	/* (non-Javadoc)
	 * @see math.geom2d.GJShape2D#transform(GJAffineTransform2D)
	 */
    public abstract GJAbstractLine2D transform(GJAffineTransform2D transform);

    public GJCurveSet2D<? extends GJAbstractLine2D> clip(GJBox2D box) {
        // Clip the curve
        GJCurveSet2D<GJContinuousCurve2D> set = GJCurves2D.clipContinuousCurve(
                this, box);

        // Stores the result in appropriate structure
        GJCurveArray2D<GJAbstractLine2D> result =
        	new GJCurveArray2D<GJAbstractLine2D>(set.size());

        // convert the result
        for (GJCurve2D curve : set.curves()) {
            if (curve instanceof GJAbstractLine2D)
                result.add((GJAbstractLine2D) curve);
        }
        return result;
    }
	...
	@Deprecated
	@Override
    public abstract GJAbstractLine2D clone();
}
