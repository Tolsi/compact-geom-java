...
public class GJPointSets2D {
	 ...
	 */
	public static <T extends GJPoint2D> boolean hasMultipleVertices(List<T> points) {
	 ...
	 */
    public static<T extends GJPoint2D> boolean hasMultipleVertices(
    	...
    	while (iter.hasNext()) {
    		current = iter.next();
    		if (GJPoint2D.distance(current, previous) < GJShape2D.ACCURACY)
    			return true;
    		previous = current;
    	}
    	return false;
     ...
     */
    public static<T extends GJPoint2D> int countMultipleVertices(
    		List<T> points) {
    	return GJPointSets2D.countMultipleVertices(points, false);
    }
	...
	public static <T extends GJPoint2D> List<T> filterMultipleVertices(
			List<T> vertices) {
    ...
    public static<T extends GJPoint2D> List<T> filterMultipleVertices(
    	...
    	// createFromCollection result with the right number of simple vertices
