...
public class GJMatrix {
    ...
    /**
     * Constructs a new GJMatrix, with 1 row and 1 column, initialized to 1.
     */
    public GJMatrix() {
        this(1, 1);
    }

    /** Initializes a new GJMatrix with the given number of rows and columns. */
    public GJMatrix(int nbRows, int nbCols) {
        nRows = nbRows;
        nCols = nbCols;
    ...
    /**
     * Constructs a new GJMatrix, initialized with the given coefficients.
     */
    public GJMatrix(double[][] coef) {
		if (coef == null) {
			nRows = 1;
			nCols = 1;
			el = new double[nRows][nCols];
     ...
     */
    public GJMatrix multiplyWith(GJMatrix matrix) {

		// check sizes of the matrices
		if (nCols != matrix.nRows)
			throw new IllegalArgumentException("GJMatrix sizes do not match");
		
		double sum;
		GJMatrix m = new GJMatrix(nRows, matrix.nCols);
     ...
     */
    public GJMatrix getTranspose() {
        GJMatrix mat = new GJMatrix(nCols, nRows);
    ...
    public double[] solve(double vector[]) {
		...
		if (vector.length != nRows) 
			throw new IllegalArgumentException("GJMatrix and vector dimensions do not match");
		
		if (nCols != nRows)
			throw new UnsupportedOperationException("Try to invert non square GJMatrix");
		...
		for (r = 0; r < nRows; r++) {
			...
			while ((Math.abs(mat.el[p][r]) < GJMatrix.tolerance) && (p < nRows))
                p++;
    ...
    /**
     * Returns a String representation of the elements of the GJMatrix
     */
    @Override
	public String toString() {
		String res = new String("");
		res = res.concat("GJMatrix size : " + Integer.toString(nRows)
