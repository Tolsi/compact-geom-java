...
public class AffineTransform2D implements Bijection2D, GeometricObject2D,
	...
	public static AffineTransform2D createRotation(double cx, double cy,
		...
		if (abs(k * PI / 2 - angle) < Shape2D.ACCURACY) {
	...
	public static boolean isIdentity(AffineTransform2D trans) {
		if (abs(trans.m00 - 1) > Shape2D.ACCURACY)
			return false;
		if (abs(trans.m01) > Shape2D.ACCURACY)
			return false;
		if (abs(trans.m02) > Shape2D.ACCURACY)
			return false;
		if (abs(trans.m10) > Shape2D.ACCURACY)
			return false;
		if (abs(trans.m11 - 1) > Shape2D.ACCURACY)
			return false;
		if (abs(trans.m12) > Shape2D.ACCURACY)
			return false;
		return true;
	}
	...
	public static boolean isIsometry(AffineTransform2D trans) {
		...
		if (abs(a * a + b * b - 1) > Shape2D.ACCURACY)
			return false;
		if (abs(c * c + d * d - 1) > Shape2D.ACCURACY)
			return false;
		
		// determinant must be -1 or +1
		if (abs(a * b + c * d) > Shape2D.ACCURACY)
			return false;
	...
	public static boolean isSimilarity(AffineTransform2D trans) {
		...
		// test each condition
		if (abs(a * a + b * b - k2) > Shape2D.ACCURACY)
			return false;
		if (abs(c * c + d * d - k2) > Shape2D.ACCURACY)
			return false;
		if (abs(a * a + c * c - k2) > Shape2D.ACCURACY)
			return false;
		if (abs(b * b + d * d - k2) > Shape2D.ACCURACY)
			return false;
